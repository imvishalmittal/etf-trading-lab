...
const dataSource = (args.source ?? process.env.MARKET_DATA_SOURCE ?? 'groww').toLowerCase();
...
if (!['groww', 'kite'].includes(dataSource)) throw new Error(`Unknown market data source ${dataSource}`);
...
function reportMarkdown(status, integrity, gate, summaries) {
  ...
  return `# ${config.strategyId} ${stage} result\n\n` +
    ...
    `- Source: ${integrity.source ?? dataSource} genuine NIFTYBEES one-minute OHLC\n` +
...
async function main() {
  ...
  let instrument;
  try {
    instrument = dataSource === 'kite'
      ? await verifyKiteInstrument({ apiKey: process.env.KITE_API_KEY, accessToken: process.env.KITE_ACCESS_TOKEN, expected: config.instrument })
      : await verifyInstrument(config.instrument);
    write('instrument.json', `${JSON.stringify(instrument, null, 2)}\n`);
  } catch (error) {
    const integrity = { source: dataSource, requested: period, observedSessions: 0, eligibleSessions: 0, rejectedSessions: 0, error: error.message };
  ...
  let fetched;
  try {
    fetched = dataSource === 'kite'
      ? await fetchKiteMinuteCandles({
      ...
      : await fetchMinuteCandles({ token: process.env.GROWW_ACCESS_TOKEN, instrument: config.instrument, start: period.start, end: period.end });
  } catch (error) {
    const integrity = { source: dataSource, requested: period, observedSessions: 0, eligibleSessions: 0, rejectedSessions: 0, error: error.message };
    const status = 'DATA_BLOCKED';
    write('data-integrity.json', `${JSON.stringify(integrity, null, 2)}\n`);
  ...
  const sourceLabel = dataSource === 'kite' ? 'Zerodha Kite historical candles' : 'Groww historical candles';
  const audited = auditCandles(fetched.candles, { ...period, source: sourceLabel });
  ...
  write('data-manifest.json', `${JSON.stringify({
    source: dataSource === 'kite' ? 'Zerodha Kite Connect' : 'Groww', requested: period, rawCsvSha256: sha256(rawCsv), compressedSha256: sha256(zlib.gzipSync(rawCsv, { level: 9 })),
